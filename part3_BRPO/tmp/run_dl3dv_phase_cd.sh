...
coarse_ok=0
if [ -f "$COARSE_ROOT/manifests/selection_summary.json" ] && [ -f "$COARSE_ROOT/pseudo_cache/manifest.json" ]; then
  first_fused=$(/home/bzhang512/miniconda3/envs/s3po-gs/bin/python - <<'PY'
import json
...
frame_ids = summary.get('selected_frame_ids', [])
if frame_ids:
    print(root / 'fusion' / 'samples' / f"{int(frame_ids[0]):04d}" / 'target_rgb_fused.png')
PY
)
  if [ -n "$first_fused" ] && [ -f "$first_fused" ]; then
    coarse_ok=1
  fi
fi

if [ "$coarse_ok" -ne 1 ]; then
...
frame_ids = summary.get('selected_frame_ids', [])
if not frame_ids:
    raise SystemExit('Coarse root has no selected_frame_ids')
first_fused = root / 'fusion' / 'samples' / f"{int(frame_ids[0]):04d}" / 'target_rgb_fused.png'
