...
def current_w2c(vp) -> torch.Tensor:
    # Fix dtype mismatch: R/T may be float64 from dataset, ensure float32
    base = torch.eye(4, device=vp.R.device, dtype=torch.float32)
    base[:3, :3] = vp.R.float()
    base[:3, 3] = vp.T.float()
...
def export_view_state(view: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if hasattr(vp, 'R0') and hasattr(vp, 'T0') and vp.R0 is not None and vp.T0 is not None:
        w2c0 = torch.eye(4, device=vp.R0.device, dtype=torch.float32)  # Fix dtype mismatch
        w2c0[:3, :3] = vp.R0
        w2c0[:3, 3] = vp.T0
