...
class FrontEnd(mp.Process):
    def __init__(self, config,model, save_dir=None):
        ...
        self.theta = 0
        self.brpo_online_mapping_enabled = bool(
            self.config.get("Results", {}).get("brpo_online_mapping", {}).get("enabled", False)
        )
    ...
    def _export_runtime_camera_state(self, frame_idx, viewpoint, *, is_keyframe=False):
        w2c = torch.eye(4, device=viewpoint.R.device, dtype=viewpoint.R.dtype)
        w2c[:3, :3] = viewpoint.R.detach()
        w2c[:3, 3] = viewpoint.T.detach()
        pose_c2w = torch.linalg.inv(w2c).detach().cpu().numpy().tolist()
        image_path = None
        if hasattr(self.dataset, "color_paths") and frame_idx < len(self.dataset.color_paths):
            image_path = str(self.dataset.color_paths[frame_idx])
        return {
            "frame_id": int(frame_idx),
            "uid": int(getattr(viewpoint, "uid", frame_idx)),
            "image_path": image_path,
            "image_name": os.path.basename(image_path) if image_path else f"{int(frame_idx):05d}.png",
            ...
            "FoVx": float(viewpoint.FoVx),
            "FoVy": float(viewpoint.FoVy),
            "pose_c2w": pose_c2w,
    ...
    def request_keyframe(self, cur_frame_idx, viewpoint, current_window, depthmap):
        ...
        if self.brpo_online_mapping_enabled:
            runtime_state_payload = dict(self.runtime_camera_state_cache)
        msg = ["keyframe", cur_frame_idx, viewpoint, current_window, depthmap, self.theta, runtime_state_payload]
    ...
    def run(self):
        ...
        while True:
            ...
            if self.frontend_queue.empty():    
                ...
                # 新添加
                force_keyframe_indices = self.config["Training"].get("force_keyframe_indices", None)
                if force_keyframe_indices is not None:
                    # 模式1：完全按列表，列表外的帧不可能是关键帧
                    create_kf = cur_frame_idx in force_keyframe_indices
                else:
                    # 模式2：原有逻辑
                    create_kf = self.is_keyframe(
                        cur_frame_idx,
