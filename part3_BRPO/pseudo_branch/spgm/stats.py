...
Outputs now include:
- support_count: accepted pseudo support (legacy active-set anchor)
- population_support_count: current train-window support summary
- depth_value: accepted-pseudo weighted median camera depth
- density_proxy: legacy opacity-support proxy
- struct_density_proxy: opacity / volume * population-support style proxy
- active_mask: still derived from accepted pseudo support for backward compatibility
"""
...
def collect_spgm_stats(
    ...
    population_support_norm = _normalize_active(population_support_count, active_mask)
    ...
    if active_mask.any():
        ...
        population_support_mean = float(population_support_count[active_mask].mean().item())
        struct_density_mean = float(struct_density_proxy[active_mask].mean().item())
    else:
        support_mean = 0.0
        support_p50 = 0.0
        support_max = 0.0
