...
VERIFICATION_VERSION = "brpo-verify-v1"


def parse_args():
    p = argparse.ArgumentParser(description="Phase C prototype: build BRPO mask from internal cache")
...
def run_branch(side, frame_id, pseudo_state, ref_state, pseudo_rgb_path, pseudo_depth_path, stage_ply, gaussians, pipe, background, matcher, tau_reproj_px, tau_rel_depth, pseudo_left_root=None, pseudo_right_root=None):
    ref_rgb_path = Path(ref_state["image_path"])
    branch_pseudo_rgb_path = resolve_branch_pseudo_rgb(side, int(frame_id), pseudo_state.get("image_name", f"{int(frame_id):05d}.png"), Path(pseudo_rgb_path), pseudo_left_root, pseudo_right_root)
...
def main():
    ...
    for frame_id in args.frame_ids:
        ...
        left_result, left_meta, left_ref_depth = run_branch(
            ...
            stage_ply, gaussians, pipe, background, matcher, args.tau_reproj_px, args.tau_rel_depth, args.pseudo_left_root, args.pseudo_right_root
        )
        ...
        write_frame_outputs(frame_out, fused, frame_meta)
