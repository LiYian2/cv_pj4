...
> 目标：记录 `pseudo_branch/` 第二轮整理的 live layout，固化已完成的 G~ direct migration，并给出后续迁移顺序。

---

> 当前进度：Phase 0（目录骨架）与 Phase 1（G~ direct migration）已完成，并已通过 `py_compile` + 最小 real import/smoke 验证；下一步进入 Phase 2（R~ 壳层迁移）。
...
### Phase 2：R~ 壳层迁移
- 移 `pseudo_loss_v2.py`
- 移 `pseudo_refine_scheduler.py`
- 移 `pseudo_camera_state.py`
- 同步改 `scripts/run_pseudo_refinement_v2.py` import

### Phase 3：T~/observation 主干迁移
...
### Phase 4：M~ 辅助文件迁移
- 把纯 confidence / mask helper 收回 `mask/`
- 收掉 `brpo_v2_signal/` 下已经被拆散的剩余平铺文件

### Phase 5：common 收尾 + 清理旧平铺路径
- 把 geometry/cache/diag helper 收进 `common/`
- 删除空目录和失效旧 import
- 刷新 docs/design 中的代码路径索引
...
当前已完成“骨架 + G~ Phase 1”两步，live G~ 入口已经收敛到 `pseudo_branch/gaussian_management/`。后续迁移继续按“R~ → T~/observation → M~ → common”推进。因为用户已经明确允许 direct migration，所以后面每一步都应直接切到新路径，不再为了最小兼容长期保留旧平铺结构。
